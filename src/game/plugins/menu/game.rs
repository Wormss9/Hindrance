...
pub fn setup_game(
    ...
    commands.insert_resource(Into::<Edges>::into(board.clone()));
    ...
    let square_entity = commands
        ...
        .with_children(|parent| {
            let board = board.clone();
            // Exit arrow
            let (x_mid, y_mid) = &board.shape.grid_mids();
            parent
                .spawn((
            ...
            parent.spawn((
                ...
                Transform {
                    translation: Vec3::new(
                        -&board.get_x_offset() * (board.grid_mids().0 + 2) as f32,
                        0.,
                        0.,
                    ),
                    ..default()
                ...
                for x in 0..x_size {
                    ...
                    for wall_position in &board.get_walls(x, y) {
                        let wall_entity = parent
                            ...
                            .id();
                        wall_entities.push((wall_entity, wall_position.clone()));
                    }
            ...
            match &board.shape {
                Shape::Square(_) => {
                    ...
                    parent.spawn((
                        ...
                        Id(board.get_id(x, y).expect("Failed to spawn foe1")),
                        Owner::Foe1,
                        Character,
                    ));
                    ...
                    parent.spawn((
                        ...
                        GridLocation::new(x, y),
                        Id(board.get_id(x, y).expect("Failed to spawn own")),
                        Owner::Own,
                        Character,
                    ));
                }
                Shape::Hexagon(_) => {
                    ...
                    parent.spawn((
                        ...
                        GridLocation::new(x, y),
                        Id(board.get_id(x, y).expect("Failed to spawn foe2")),
                        Owner::Foe2,
                        Character,
                    ));
                    let (x, y) = (board.size(), 2 * board.size() - 1);
